...
void USART1_Config(unsigned int u32_Baud , unsigned char UART_IT_EN)
{
	  ...
	  USART_ITConfig(USART1, USART_IT_IDLE, ENABLE);//开启两帧间隔中断
...
void USART2_Config(unsigned int u32_Baud , unsigned char UART_IT_EN)
{
	  ...
	  RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);//使能串口二模块时钟
 
		/* USART1 GPIO config */
		/* Configure USART2 Tx (PA.02) as alternate function push-pull */
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//设置引脚模式
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(GPIOA, &GPIO_InitStructure);
		...
		USART_InitStructure.USART_BaudRate =u32_Baud ;//配置串口数据发送信息
		USART_InitStructure.USART_WordLength = USART_WordLength_8b;
...
void USART2_NVIC_Configuration(void)/// 配置USART2接收中断
{
	NVIC_InitTypeDef NVIC_InitStructure; 
	/* Configure the NVIC Preemption Priority Bits */  
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_0);//选着中断组
...
void USART3_Config(unsigned int u32_Baud , unsigned char UART_IT_EN)
{
	  ...
	  RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3, ENABLE);//使能串口3模块时钟
 
		/* USART3 GPIO config */
		/* Configure USART3 Tx (PB.10) as alternate function push-pull */
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;//设置引脚模式
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
void UART4_Config(unsigned int u32_Baud , unsigned char UART_IT_EN)
{
	  ...
	  RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART4, ENABLE);//使能串口4模块时钟
	  ...
	  if(UART_IT_EN==1)
        ...
        USART_ITConfig(UART4, USART_IT_RXNE, ENABLE);/* 使能串口4接收中断 */
			  ...
			  USART_ITConfig(UART4, USART_IT_RXNE, DISABLE);/*禁止串口4接收中断 */
...
void UART5_Config(unsigned int u32_Baud , unsigned char UART_IT_EN)
{
	  ...
	  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);//使能端口时钟
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);//使能端口时钟
	  RCC_APB1PeriphClockCmd(RCC_APB1Periph_UART5, ENABLE);//使能串口5模块时钟
 
		/* USART5 GPIO config */
		/* Configure USART5 Tx (PC.12) as alternate function push-pull */
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;//设置引脚模式
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(GPIOC, &GPIO_InitStructure);
	  ...
	  if(UART_IT_EN==1)
		{
			  USART5_NVIC_Configuration();
        USART_ITConfig(UART5, USART_IT_RXNE, ENABLE);/* 使能串口5接收中断 */
    }
		else 
			  USART_ITConfig(UART5, USART_IT_RXNE, DISABLE);/*禁止串口5接收中断 */
		USART_Cmd(UART5, ENABLE);//
}
void USART5_NVIC_Configuration(void)///  
{
	NVIC_InitTypeDef NVIC_InitStructure; 
	/* Configure the NVIC Preemption Priority Bits */  
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_0);//选着中断组
