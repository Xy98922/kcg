...
void PWM_config_Init(u8 u8PWMx , u16 u16channel , u32 u32period , u16 u16duty)
{
	    ...
	    TIM_OCInitTypeDef  TIM_OCInitStructure;  
			...
			{    
						...
						TIM_TimeBaseStructure.TIM_Period =719;       //当定时器从0计数到999，即为1000次，为一个定时周期
						TIM_TimeBaseStructure.TIM_Prescaler =1999;	    //设置预分频：不预分频，即为72MHz
						...
						{              
							...
							TIM_OCInitStructure.TIM_Pulse = u16duty;	   //设置跳变值，当计数器计数到这个值时，电平发生跳变
		...
		else if(u8PWMx == PWM2)//使用定时器4 输出比较功能
		{
            ...
            TIM_TimeBaseStructure.TIM_Period =719;       //0.4*度数+18=比较值 
						TIM_TimeBaseStructure.TIM_Prescaler =1999;	    //50Hz
						TIM_TimeBaseStructure.TIM_ClockDivision = 0;	//设置时钟分频系数：不分频(这里用不到)
						TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //向上计数模式
          ...
          if(u16channel == PWM_Channel_0)
					{              
						...
						TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;	
						TIM_OCInitStructure.TIM_Pulse =u16duty;	   //设置跳变值，当计数器计数到这个值时，电平发生跳变
						TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;  //当定时器计数值小于CCR1_Val时为高电平，大舵机需要反向，改为高
