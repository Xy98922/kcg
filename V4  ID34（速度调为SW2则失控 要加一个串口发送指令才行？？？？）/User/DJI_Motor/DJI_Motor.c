...
void DiffX2_Wheel_Rpm_Model(float v1,float v2)
{
	...
	setv2 = -v2;
	setv1 = -v1;	
	UART_send_string(USART2,"YIAKONGQISET:  ");UART_send_intdata(USART2,setv1);UART_send_char(USART2,'\t');
//	if(MS_Flag==0){
//		 send_loradata();
...
void DJI_Motor_Control(void)
{
	...
	{  DATA[0]=0x3E;
     ...
     DATA[5] = (int)setv1&0xff;
	  ...
	  UART_send_string(USART2,"v1:");UART_send_intdata(USART2,test);UART_send_char(USART2,'\t');	
...
void servoinit(int16_t angle1)
{
  ...
  PWM_config_Init(PWM2, PWM_Channel_0 , 0, 0);
	...
	TIM_SetCompare1(TIM4,54);      // PB6,28
