...
/**
  ...
  */
/*************************************************************************
*  函数名称：Timer_2to7_counter_Generalfuncation
*  功能说明：STM32定时中断基本功能配置函数 只具备定时产生中断功能
*  参数说明：TIMx        模块号（TIM2、TIM3、TIM4、TIM5、TIM6、TIM7）
*            u16counter   定时时间参数（定时时间 == 1us * u16counter）
*  函数返回：无
*  修改时间：2014-11-3
*  备    注：CRP  由于STM32有8个定时器   但是定时器1 及 定时器8 属于高级定时器
*                 考虑到基本定时器有6 个已经足够使用  所以基本功能中没有包含定时1、8
*************************************************************************/	
void Timer_2to7_counter_Generalfuncation(TIM_TypeDef* TIMx,u16 u16counter)
{
	...
	else if(TIMx == TIM6)
	{
      NVIC_PriorityGroupConfig(NVIC_PriorityGroup_0);  													
		  ...
		  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM6 , ENABLE);/* 设置TIM6CLK 为 72MHZ */		
     //TIM_DeInit(TIM2);	
	   /* 自动重装载寄存器周期的值(计数值) */
			TIM_TimeBaseStructure.TIM_Period=u16counter-1;//0~65536
			TIM_TimeBaseStructure.TIM_Prescaler= 71; /* 时钟预分频数为72 */	
			...
			TIM_ClearFlag(TIM6, TIM_FLAG_Update);//清除向上计数中断标志位	
	...
	else if(TIMx == TIM7)
	{
      NVIC_PriorityGroupConfig(NVIC_PriorityGroup_0);  													
		  ...
		  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM7 , ENABLE);/* 设置TIM7CLK 为 72MHZ */		
     //TIM_DeInit(TIM2);	
	   /* 自动重装载寄存器周期的值(计数值) */
			TIM_TimeBaseStructure.TIM_Period=u16counter-1;//0~65536
			TIM_TimeBaseStructure.TIM_Prescaler= 71; /* 时钟预分频数为72 */	
...
/*************************************************************************
*  函数名称：Timer_2to7_Generalfuncation_start
*  功能说明：STM32定时中断基本功能 启动计数函数
*  参数说明：TIMx        模块号（TIM2、TIM3、TIM4、TIM5、TIM6、TIM7）
*             
*  函数返回：无
*  修改时间：2014-11-3
*  备    注：CRP   
*************************************************************************/
void Timer_2to7_Generalfuncation_start(TIM_TypeDef* TIMx)
{
   if(TIMx == TIM2)
	 {
...
/*************************************************************************
*  函数名称：Timer_2to7_Generalfuncation_stop
*  功能说明：STM32定时中断基本功能 停止计数函数
*  参数说明：TIMx        模块号（TIM2、TIM3、TIM4、TIM5、TIM6、TIM7）
*             
*  函数返回：无
*  修改时间：2014-11-3
*  备    注：CRP   
*************************************************************************/
void Timer_2to7_Generalfuncation_stop(TIM_TypeDef* TIMx)
{
   if(TIMx == TIM2)
	 {
